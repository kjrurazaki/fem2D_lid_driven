...
def run_2D(model, method):
    # Impose BCs
    (stiffMat, rhs, u_dir_velocity, u_dir_pressure, boundary_nodes_velocity) = imposeBC(
        model, method=method
    )
...
if __name__ == "__main__":
    ...
    type_list = [
        ...
        "bubble",
        "p1-iso-p2",
    ]  # ['linear', 'linear_gls', 'bubble', 'p1-iso-p2']
        ...
        for type in type_list:
            for delta in delta_list:
                if delta is not None and type != "linear_gls":
                    ...
                    model_lid = Model(
                        meshdir, delta=delta, only_dirichlet=True, element_type=type
                    )

                    uh, p, residual, residual_boundary, rhs = run_2D(
                        model_lid, method=method
                    )
                    ...
                    pd.DataFrame(model_lid.coord).to_csv(
                        f"./Models/coord_i{i}_type{type}_delta{delta}.csv"
                    )
                    pd.DataFrame(model_lid.triang_velocity).to_csv(
                        f"./Models/triangv_i{i}_type{type}_delta{delta}.csv"
                    )
                    pd.DataFrame(model_lid.triang_pressure).to_csv(
                        f"./Models/triangp_i{i}_type{type}_delta{delta}.csv"
                    )
                    pd.DataFrame(model_lid.uh).to_csv(
                        f"./Models/v_i{i}_type{type}_delta{delta}.csv"
                    )
                    pd.DataFrame(model_lid.p).to_csv(
                        f"./Models/p_i{i}_type{type}_delta{delta}.csv"
                    )
                    pd.DataFrame(model_lid.element.h).to_csv(
                        f"./Models/h_i{i}_type{type}_delta{delta}.csv"
                    )
    ...
    pd.DataFrame.from_dict(
        {
            ...
            "residual_bc": residuals_bc_values,
        }
    ).to_csv(f"./Models/residuals.csv")
